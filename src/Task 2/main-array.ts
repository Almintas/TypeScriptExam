...
type Person = {
  ...
  console.group('Užduotys');
  {
    ...
    const sameString = (word: string, name: string) => word + name[0];

    const createFunction = (word: string[]): string => {
        const create = word.reduce<string>(sameString, '');
        return create;
    }
    console.groupEnd();
    ...
    type NumbersMultiply = (product: number, number: number) => number;

    type Multiply = (number: Array<number>) => number;

    const numbersMultiply: NumbersMultiply = (product, number) => product * number;

    const multiply: Multiply = (number) => number.reduce<number>(numbersMultiply, 1);
    console.groupEnd();
  }
  console.groupEnd();