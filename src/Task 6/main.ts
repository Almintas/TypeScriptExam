...
{
    const task1 = <Type>(arr: Type[]): Type => {
        return arr[0];
}
console.groupEnd();
...
{
    const task2 = <Type>(arr: Type[]): Type => {
...
{
    const task3 = <Type extends PrimitiveType>(arr: Type[]): Type[] => {
        const copy: Type[] = arr.map(x => x);
        return copy;
}

console.log({ numbers, result: task3<number>(numbers) });
console.log({ strings, result: task3<string>(strings) });
console.log({ booleans, result: task3<boolean>(booleans) });
